...
if __name__ == '__main__':
    ...
    y = canvas.new_rectangle(
        lower_left = x.vertices()[0],
        upper_right = x.vertices()[0] + GRID_SIZE,
    ).get_skeleton()
    ...
    p = centroid([ret_g.vertices()[1], ret_g.vertices()[2]]) + V_OFFSET
    ...
    p = centroid([y.vertices()[1], y.vertices()[2]]) + V_OFFSET
    canvas.new_arrow(src = p, dst = y)
    canvas.new_text(
        text = 'y',
        anchor = p,
        location = locations.EAST,
